#!/usr/bin/env python3
"""Fast-lane clause classifier evaluation.

Scores the fast-lane against:
  1. data/annotated/leivaditi_redflags.jsonl  -- 738 hand-labeled sentences
     (7 gold taxonomy types); reports per-class P/R/F1 + unknown rate.
...
from legalrag.extract.fast_lane import classifyClause
...
def main() -> int:
    if REDFLAGS.is_file():
        ...
        print("[eval_fastlane] gold redflags:", json.dumps(report, indent=2))
